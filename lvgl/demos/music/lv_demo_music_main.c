...
lv_obj_t * _lv_demo_music_main_create(lv_obj_t * parent)
{
    ...
    create_wave_images(cont);       /*创建波形图*/
    lv_obj_t * title_box = create_title_box(cont);  /*创建标题容器*/
    lv_obj_t * icon_box = create_icon_box(cont);    /*创建图标容器*/
    lv_obj_t * ctrl_box = create_ctrl_box(cont);    /*创建控制容器*/
    spectrum_obj = create_spectrum_obj(cont);    /*创建频谱容器*/
    lv_obj_t * handle_box = create_handle(cont);    /*创建手柄容器*/
...
# endif
...
#else
    /*Arrange the content into a grid*/
    static const lv_coord_t grid_cols[] = {LV_GRID_FR(1), LV_GRID_FR(1), LV_GRID_TEMPLATE_LAST};    /*列*/
    static const lv_coord_t grid_rows[] = {LV_DEMO_MUSIC_HANDLE_SIZE,     /*Spacing*/   /*行*/
                                           LV_GRID_FR(1),   /*Spacer*/  /*行*/
                                           LV_GRID_CONTENT, /*Title box*/
                                           LV_GRID_FR(1),   /*Spacer*/  /*行*/
                                           LV_GRID_CONTENT, /*Icon box*/
                                           LV_GRID_FR(3),   /*Spacer*/
                                           ...
                                           LV_GRID_CONTENT, /*Handle box*/
                                           LV_GRID_FR(1),   /*Spacer*/
                                           LV_DEMO_MUSIC_HANDLE_SIZE,     /*Spacing*/   /*行*/
                                           LV_GRID_TEMPLATE_LAST};

    lv_obj_set_grid_dsc_array(cont, grid_cols, grid_rows);  /*设置网格*/
    lv_obj_set_style_grid_row_align(cont, LV_GRID_ALIGN_SPACE_BETWEEN, 0);  /*设置网格行对齐方式*/
    lv_obj_set_grid_cell(title_box, LV_GRID_ALIGN_STRETCH, 0, 1, LV_GRID_ALIGN_CENTER, 2, 1);   /*设置网格单元格*/
    lv_obj_set_grid_cell(icon_box, LV_GRID_ALIGN_STRETCH, 0, 1, LV_GRID_ALIGN_CENTER, 4, 1);    
    ...
    sec_counter_timer = lv_timer_create(timer_cb, 1000, NULL);  /*创建定时器*/
    lv_timer_pause(sec_counter_timer);  /*暂停定时器*/

    /*Animate in the content after the intro time*/
    lv_anim_t a;    /*动画*/

    start_anim = true;  /*设置开始动画标志*/

    lv_timer_t * timer = lv_timer_create(stop_start_anim, INTRO_TIME + 6000, NULL); /*创建定时器*/
    lv_timer_set_repeat_count(timer, 1);    /*设置定时器重复次数*/

    lv_anim_init(&a);   /*初始化动画*/
    lv_anim_set_path_cb(&a, lv_anim_path_bounce);   /*设置动画路径*/

    uint32_t i;
    lv_anim_set_exec_cb(&a, start_anim_cb); /*设置动画执行回调函数*/
    for(i = 0; i < BAR_CNT; i++)    
    {
        lv_anim_set_values(&a, LV_HOR_RES, 5);  /*设置动画起始值和结束值*/
        lv_anim_set_delay(&a, INTRO_TIME - 200 + rnd_array[i] % 200);   /*设置动画延时*/
        lv_anim_set_time(&a, 2500 + rnd_array[i] % 500);    /*设置动画时间*/
        lv_anim_set_var(&a, &start_anim_values[i]);   /*设置动画变量*/
        lv_anim_start(&a);  /*开始动画*/
    }

    lv_obj_fade_in(title_box, 1000, INTRO_TIME + 1000); /*淡入*/
    lv_obj_fade_in(icon_box, 1000, INTRO_TIME + 1000);  /*淡入*/
    lv_obj_fade_in(ctrl_box, 1000, INTRO_TIME + 1000);  /*淡入*/
    ...
    lv_anim_set_var(&a, album_img_obj); /*设置动画变量*/
    lv_anim_set_time(&a, 1000); /*设置动画时间*/
    lv_anim_set_delay(&a, INTRO_TIME + 1000);   /*设置动画延时*/
...
static lv_obj_t * create_cont(lv_obj_t * parent)
{
    ...
    main_cont = lv_obj_create(parent);      /*创建一个容器*/ 
    lv_obj_clear_flag(main_cont, LV_OBJ_FLAG_CLICKABLE);        /*清除容器的可点击标志*/
    lv_obj_clear_flag(main_cont, LV_OBJ_FLAG_SCROLL_ELASTIC);   /*清除容器的弹性滚动标志*/
    lv_obj_remove_style_all(main_cont);                             /*移除容器的所有样式*/
    lv_obj_set_size(main_cont, lv_pct(100), lv_pct(100));        /*设置容器的大小*/
    lv_obj_set_scroll_snap_y(main_cont, LV_SCROLL_SNAP_CENTER);     /*设置容器的垂直滚动快照*/

    /*Create a container for the player*/
    lv_obj_t * player = lv_obj_create(main_cont);     /*创建一个容器*/
    lv_obj_set_y(player, - LV_DEMO_MUSIC_HANDLE_SIZE);  /*设置容器的Y坐标*/
#if LV_DEMO_MUSIC_SQUARE || LV_DEMO_MUSIC_ROUND
    lv_obj_set_size(player, LV_HOR_RES, 2 * LV_VER_RES + LV_DEMO_MUSIC_HANDLE_SIZE * 2);
#else
    lv_obj_set_size(player, LV_HOR_RES, LV_VER_RES + LV_DEMO_MUSIC_HANDLE_SIZE * 2);    /*设置容器的大小*/
#endif
    lv_obj_clear_flag(player, LV_OBJ_FLAG_SNAPABLE);    /*清除容器的可捕捉标志*/

    lv_obj_set_style_bg_color(player, lv_color_hex(0xffffff), 0);   /*设置容器的背景颜色*/
    lv_obj_set_style_border_width(player, 0, 0);    /*设置容器的边框宽度*/
    lv_obj_set_style_pad_all(player, 0, 0);     /*设置容器的内边距*/
    lv_obj_set_scroll_dir(player, LV_DIR_VER);      /*设置容器的滚动方向*/

    /* Transparent placeholders below the player container
     * It is used only to snap it to center.*/
    lv_obj_t * placeholder1 = lv_obj_create(main_cont);  /*创建一个容器*/
    lv_obj_remove_style_all(placeholder1);  /*移除容器的所有样式*/
    lv_obj_clear_flag(placeholder1, LV_OBJ_FLAG_CLICKABLE); /*清除容器的可点击标志*/
//    lv_obj_set_style_bg_color(placeholder1, lv_color_hex(0xff0000), 0);
//    lv_obj_set_style_bg_opa(placeholder1, LV_OPA_50, 0);

    lv_obj_t * placeholder2 = lv_obj_create(main_cont); /*创建一个容器*/
    lv_obj_remove_style_all(placeholder2);  /*移除容器的所有样式*/
    lv_obj_clear_flag(placeholder2, LV_OBJ_FLAG_CLICKABLE); /*清除容器的可点击标志*/
...
#else
    lv_obj_set_size(placeholder1, lv_pct(100), LV_VER_RES); /*设置容器的大小*/
    lv_obj_set_y(placeholder1, 0);  /*设置容器的Y坐标*/

    lv_obj_set_size(placeholder2, lv_pct(100),  LV_VER_RES - 2 * LV_DEMO_MUSIC_HANDLE_SIZE);    /*设置容器的大小*/
    lv_obj_set_y(placeholder2, LV_VER_RES + LV_DEMO_MUSIC_HANDLE_SIZE);   /*设置容器的Y坐标*/
#endif

    lv_obj_update_layout(main_cont);    /*更新容器的布局*/
